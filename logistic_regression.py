...
class logistic_regressor(object):
    ...
    def cost_function(self, Theta):
        ...
        return sum(self.y*np.log(self.logistic(np.dot(X2,Theta))) + \
                   (1-self.y)*np.log(1-self.logistic(np.dot(X2,Theta))))
    
    def gradient_descent(self, Theta, plot = None):
        m = self.y.size
        X2 = np.column_stack([np.ones(m), self.X])
        for i in np.arange(1,self.iterations):
            Theta += self.alpha*np.dot((self.y - self.logistic(np.dot(X2, Theta))), X2)
        if plot == 'on':
